\chapter{The LHC and its CMS Detector} % (fold)
...
\begin{figure}[!htbp]
	...
	\caption{The integrated luminosity of the LHC with proton-proton collisions in 2016 compared to previous years~\cite{Pralavorio2016}. The recorded luminosity in 2016 far surpassed the expected luminosity and its almost double the luminosity recorded in 2012.\todo[inline]{explain each line}}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.75\textwidth]{figures/LHC/CMS_magnet.jpg}
...
\begin{table}
\begin{tabular}[!htbp]{l p{0.5\linewidth}}
\hline
{\bf Trigger Path} & {\bf Explanation} \\
\hline
HLT\_Ele27\_WPTight\_Gsf\_v7 & Requires at least one electron at HLT level having $p_T$ of 27 GeV along with some tight selection on kinematics variable. The electron is reconstructed using Gaussian Sum Filter (GSF) technique\footnote{In CMS detector we use large amount of material that includes detector itself along with the on detector electronics, mechanical support and power cooling. Thus along with the ionization energy loss and multiple coulomb scattering it also suffers from energy loss due to bremsstrahlung. Thus, to  precisely model the track, we have to include the material effects. This is done using the linear generalization of Kalman Filter\cite{Kalman1960}, the GSF\cite{Fruehwirth1998,Fruehwirth1997} in the track reconstruction. It assumes all the different components of different degree of the bremsstrahlung under consideration as the sum of several Gaussian mixtures~\cite{Adam2005}.}.\\
\hline
HLT\_Ele27\_eta2p1\_WPLoose\_Gsf\_v8 & Requires at least one trigger at HLT level having $p_T$ greater then 27 GeV having loose selection on kinematics variable. In addition to this electron should be within pseudo-rapidity region of 2.1.\\
\hline
HLT\_Ele27\_eta2p1\_WPTight\_Gsf\_v8 & This is similar to the previous trigger except here tight working point is applied instead of loose.\\
\hline
HLT\_IsoMu24\_v 	& Requires at least one isolated muon at HLT level having $p_T$ greater than 24 GeV.	\\
\hline 
HLT\_IsoTkMu24\_v & Requires at least one isolated muon at HLT with $p_T$ greater than 24 GeV which is also matched with the tracker muon signature. \\	
\end{tabular}
\caption{Example of several electron and muons HLT trigger paths used in 2016.}
\label{tab:trigger-path}
\end{table}

All the events that passes HLT path are stored in local disk (in CMS its Tier 0\footnote{The Worldwide LHC Computing Grid (WLCG) is composed of four levels, or “Tiers”, identified with numbers 0, 1, 2 and 3. Each Tier is made up of several computer centres and provides a specific set of services; they process, store and analyse all the data from the LHC. Tier 0 is the CERN Data Centre. All of the data from the LHC pass through this central hub. Tier 0 distributes the raw data and the reconstructed output to Tier 1’s, and reprocesses data when the LHC is not running.})

% \subsubsection{Example of one of HLT Trigger For Electrons} % (fold)
% \label{ssub:example_of_one_of_hlt_trigger_for_electrons}
% Here, I am explaining in details one of triggers, \textbf{HLT_Ele27_WPTight_Gsf_v7}, from the HLT trigger menu. 
% subsubsection example_of_one_of_hlt_trigger (end)

% \subsubsection{Example: Trigger for Muons} % (fold)
% \label{ssub:example_trigger_for_muons}
% As described in section~\ref{sub:the_muon_system}, muons are signature of many interesting physics as it contains leptons isolated from many hadronic activity. While the dominant component of muon trigger rate consists of muons from hadron decays. This can be exploited by applying isolation criteria and lowering the $p_T$ threshold. In this way we can optimize the physics reach of CMS detector. Also, muons are used to calibrate the detectors.
% % Also, in section~\ref{ssub:l1_muon_trigger} it is described.

% As we discussed earlier, at Level-1 upto four muons candidates passes the trigger that exceeds the transverse momentum ($p_T$) threshold and matches muon candidates of DT and CSC along with the RPC candidates. Thus, at HLT we use several different HLT for muons as the type of event that contribute depends on the $p_T$ threshold used. Like, Pion and Kaon decays dominate in the region $p_T<5GeV$. Leptons from b and c-quark decay dominates in the $5~GeV<p_T<35~GeV$ range while at higher $p_T$ values, $W\rightarrow \mu \nu$ or similar topology, events are the main component of the muons passing the Level-1 criteria.


% % subsubsection example_trigger_for_muons (end)

% \subsubsection{Example: Trigger for Electrons} % (fold)
% \label{ssub:example_trigger_for_electrons}

% subsubsection example_trigger_for_electrons (end)
...
Once the events are selected by the trigger system and data are stored to disk. They are ready to be analysed offline. Before that, the raw data need to be processed, i.e., to convert data into understandable physics objects like electrons, muons, photons, jets, and so on. This step is known as object reconstruction which is the most CPU intensive task in the data processing chain of CMS. In this step one needs to reconstruct the primary vertices, charged particle tracks, identify electrons, photons, muons, reconstruct jets, apply b-tagging algorithm to reconstruct b-jets, run detector specific filtering and so on.
...
To analyse data, we also need MC simulations which are carried out based on the predictions of SM and various new physics models. MC events are generated at parton level using various monte-carlo generators like MadGraph or Phthia. Then showering and hadronization is applied, and finally, these events are put through the GEANT4~\cite{Agostinelli2003} based CMS detector simulation. The result is data similar to what one obtains from the actual detector. This MC data can then also be reconstructed as if it were  detector data.
