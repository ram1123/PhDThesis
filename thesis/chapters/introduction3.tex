\chapter{Introduction}\label{ch:second}
...
These renormalizable gauge theories allow the presence of spin 0 (scalar) particles, too \cite{article:higgs}. The usual formulation of the Standard Model requires at least one of these, the Higgs scalar boson, arising from the mechanism for spontaneously breaking the $SU(2)\times U(1)$ electroweak symmetry and giving masses to the weak gauge bosons and to the leptons and quarks. This mechanism is a vital part of the {sm} \cite{book:barger, book:Braibant, book:FrankClose, book:Griffiths}.
...
For $\mu^2 < 0$ the potential $V(\phi)$ has not minima at $\phi_0 = 0$ but at $\phi_1~and~\phi_2$. Now the system has two states with lowest energy with $V_{min} = - \frac{v^4 \lambda}{4}<0$ at
	\begin{equation}
	\phi_1 = \sqrt{- \frac{\mu^2}{\lambda}} = v > 0,~~and~~\phi_2 = - \sqrt{- \frac{\mu^2}{\lambda}} =-v
	\end{equation}
Now, lets expand the lagrangian around the vacuum state $\Braket{\phi}$
	\begin{equation}
	\phi(x)=\frac{1}{\sqrt{2}}[v+\phi_1(x)+i\phi_2(x)]
	\end{equation}
If we substitute this in equation (\ref{lag}) then we get
	\begin{equation}
	L=-\frac{1}{4}F^{\mu \nu}F_{\mu \nu}+\frac{1}{2}g^2v^2A^{\mu}A_{\mu}+\frac{1}{2}(\partial^{\mu}\phi_1)^2+\mu^2\phi_1^2+\frac{1}{2}(\partial^{\mu}\phi_2)^2+gvA_{\mu}\partial^{\mu}\phi_2+...
	\end{equation}
The Lagrangian $L_{SM}$ of the SM can be divided into four different parts,
	\begin{equation}
	L_{SM}=L_{YM}+L_{ferm}+L_H+L_{Yuk}
	\end{equation}
The first two part contains the dynamics of gauge bosons and matter fermions. Third part is the Higgs contribution and the fourth one is the yukawa coupling and gives masses to fermions.
The Yang-Mills part $L_{YM}$ describes the dynamics of the gauge fields, i.e. their free propagation as well as their self-interactions,
	\begin{equation}
	L_{YM}=-\frac{1}{4}W^i_{\mu \nu}W^{i, \mu \nu}-\frac{1}{4}B_{\mu \nu} B^{\mu \nu} -\frac{1}{4}G^a_{\mu \nu}G^{a,\mu \nu},
	\end{equation}
where
	\begin{eqnarray}
	...
	\end{eqnarray}
are the field-strength tensors of the gauge fields $W^i_{\mu}$ for the SU(2) group of the weak isospin $I^i_w$, $B_{\mu}$ for the $U(1)_Y$ of weak hypercharge $Y_w$, and $G^a_{\mu}$ for the SU(3) of color.
\end{comment}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{\bf Electroweak symmetry breaking and the Higgs mechanism:} \cite{article:LEPtoLHC, article:abd, article:tasi}
The Higgs part
	\begin{equation}
